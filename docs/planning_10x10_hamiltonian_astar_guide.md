...
`--mask` 时，10×10 训练才会在每一步启用“安全动作认证器”；独立的 10×10 推理入口
始终使用同一套认证规则：

1. Hamiltonian 环负责维护一个长期不把蛇头和蛇尾次序打乱的不变量；
2. A* 尝试寻找更短的吃食物路径；
3. 路径必须经过真实蛇身模拟、Hamiltonian 顺序、饥饿边界和吃完后的蛇尾可达检查；
4. 规划器得到安全动作集合后，DQN 才能在集合内探索或选择 Q 值最大的动作。
...
Masked checkpoint 额外保存 `mask_enabled=true`、掩码版本和规划器名称。Masked agent 拒绝
把普通 checkpoint 当成 masked checkpoint 恢复；普通 DQN checkpoint 的结构保持不变。
...
对单个首动作，完整状态搜索只在快速路径不能给出证明时运行。推理策略还能复用已提交
路径，因此通常比不带历史承诺的 masked 训练认证便宜。实际速度主要取决于 CPU、蛇长、
需要动态重试的动作数和 `--mask-max-astar-expansions`；GPU 只加速神经网络，不能消除 A*
的 CPU 搜索成本。降低 expansion 上限会更快，但也会使规划器更保守，不能把搜索超时的
动作当成安全。

开发验证还包括真实 checkpoint 满盘推理、10 步 masked 训练和两次同种子 6×6 普通训练
张量逐项对比。最终项目全量测试为 199 个，
