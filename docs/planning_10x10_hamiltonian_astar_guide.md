# 10×10 推理规划器：Hamiltonian、tail-safe A* 与 DQN 如何配合

## 1. 先说结论

这个分支没有重新训练 DQN，也没有修改原来的 6×6 AI。它只在 10×10 推理时给
DQN 加了一层“安全动作认证器”：

1. Hamiltonian 环负责维护一个长期不把蛇头和蛇尾次序打乱的不变量；
2. A* 尝试寻找更短的吃食物路径；
3. 路径必须经过真实蛇身模拟、Hamiltonian 顺序、饥饿边界和吃完后的蛇尾可达检查；
4. 规划器得到安全动作集合后，DQN 才能在集合内选择 Q 值最大的动作。
...
`A_safe(s)` 是规划器认证通过的动作集合。原 DQN 对全部三个动作的最大值只用于记录
“规划器是否覆盖了 DQN”，不能绕过安全集合。
...
flowchart LR
    ...
    SNAP --> HAM["Hamiltonian 顺序守卫"]
    HAM --> ASTAR["A* 候选 + 完整模拟"]
    ASTAR --> SAFE["认证动作集合 A_safe"]
    OBS["Hybrid observation"] --> DQN["现有 DQN 输出 3 个 Q 值"]
    SAFE --> MASK["只比较安全动作的 Q 值"]
    DQN --> MASK
    MASK --> ACT["执行动作并保存已认证路径后缀"]
    ACT --> ENV
```
...
因此，当快速候选和上一帧已认证路径都不能提供动作时，规划器才启用完整状态 A*：
...
完整状态搜索明显更贵，所以它不是每帧运行。超过 500 次扩展不是把动作判成安全，而是
表示“本次有界搜索没有给出证明”，随后只能检查 Hamiltonian 安全基线路径。
...
若快速 A*、完整状态 A* 和已认证路径后缀都没有提供路径，规划器沿固定环后继生成一条
到当前食物的确定路径。该路径仍要经过：
...
后缀并把它加入安全候选。如果新搜索找到更短的认证路径，DQN 仍可在新的安全集合内选择；
被选中的新路径又会成为新的承诺。
...
- DQN 网络结构和参数；
- 10×10 checkpoint；
- Double/Dueling DQN 训练逻辑；
- replay buffer、损失函数、target network；
...
为了防止每帧在多条路线之间左右摇摆，规划器在多个安全食物路径中只暴露“总长度最短”
的那些首动作，然后 DQN 在这些动作中按 Q 值选择。Q 值相同时按较小动作编号稳定选择。
...
| `planning_10x10/hamiltonian.py` | 固定环、环编号顺序、环后继路径 |
...
和逐局 CSV；其中会记录 DQN 被覆盖比例、Hamiltonian 基线路径比例、平均与最大决策耗时。

## 13. 计算量与训练速度

这个实现只接在独立的 10×10 推理入口，不在 `train.py` 的每个训练 step 中运行，所以不会
降低现有 6×6 或 10×10 DQN 的训练速度。它增加的是推理期决策时间。

设蛇长为 `n`，动态 A* 最大扩展数为 `E=500`：

- Hamiltonian 一步守卫最多检查 3 个动作，当前实现的身体模拟约为 `O(n)`；
- 快速 A* 的几何状态最多约为 `4×10×10=400` 个，优先队列操作约为 `O(V log V)`；
- 一条长度为 `L` 的候选路径逐步模拟约为 `O(Ln)`；
- 最坏的完整状态 A* 约为 `O(3En + E log E)`，并保存至多 `O(En)` 量级的蛇身状态。

完整状态搜索只在快速路径和路径承诺都不能给出证明时运行。本机对现有 10×10 `best.pt`
的一次 97 分满盘评估中，默认 `E=500` 时平均端到端决策约 7.4 ms，最慢搜索帧约 0.20 s；
数值会随 CPU、蛇长、种子和 PyTorch 设备变化。这个指标包含 DQN 前向和规划器，而不只是
A*。

开发验证还包括多个固定种子的真实 checkpoint 满盘运行。最终项目全量测试为 182 个，
其中 10×10 专项测试覆盖：环境差分、尾格释放、进食增长、环闭合与顺序、饥饿边界、
A* 假阴性反例、路径承诺、无安全动作时禁止回退、checkpoint 元数据和真实推理循环。
